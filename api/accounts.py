...
class Sub2APIExportRequest(BaseModel):
    local_account_ids: list[str] = Field(default_factory=list)
...
def create_router() -> APIRouter:
    ...
    @router.post("/api/sub2api/servers/{server_id}/export")
    async def sub2api_server_export(server_id: str, body: Sub2APIExportRequest, authorization: str | None = Header(default=None)):
        require_admin(authorization)
        server = sub2api_config.get_server(server_id)
        if server is None:
            raise HTTPException(status_code=404, detail={"error": "server not found"})
        try:
            job = sub2api_import_service.start_export(server, body.local_account_ids)
        except ValueError as exc:
            raise HTTPException(status_code=400, detail={"error": str(exc)}) from exc
        return {"import_job": job}
